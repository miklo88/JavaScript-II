...
// Problem 1 .forEach() //for each who donated and what amount they donated and their email. Then putting them in a new array companyDonations
let companyDonations = [];

runners.forEach(runner => companyDonations.push(`${runner.company_name} ${runner.donation} ${runner.email}`));
console.log(companyDonations);

// Problem 2 .map() for all runners last names change them to lower case. 
let allLower = []; 

runners.map(runner => allLower.push(runner.last_name.toLowerCase()));
console.log(allLower);

// Problem 3 .filter() going from large shirts selected to small shirts and the runners information with it are filtered. / ask about multiple elements/items filered.
let smallShirts = [];

runners.filter(runner => {if (runner.shirt_size === 'S')
